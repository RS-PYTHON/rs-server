...
def get_all_products(skip: int = 0, limit: int = 100, db=Depends(get_db)):
    return db.query(CaduProduct).offset(skip).limit(limit).all()


@router.get("/id={product_id}", response_model=CaduProductRead)
def get_product_by_id(product_id: int, db=Depends(get_db)):
    ret = db.query(CaduProduct).filter(CaduProduct.id == product_id).first()
    if ret is None:
...
def get_product_by_name(product_name: str, db=Depends(get_db)):
    ret = db.query(CaduProduct).filter(CaduProduct.name == product_name).first()
    if ret is None:
...
def create_product(product: CaduProductCreate, db=Depends(get_db)):
    ret = CaduProduct(**product.model_dump(exclude_unset=True))  # create model from the schema values
