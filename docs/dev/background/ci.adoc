...
The continuous integration is implemented with github actions.
...
It verifies that :
* the code is well formatted (with black and isort)
* the code is well linted (with ruff and flake8)
* the typing is correct (with mypy)
* there is no common vulnerability (with bandit and safety)
* there is no license issue (with safety)
* the unit tests run succesfully (with pytest)
* the integration tests run succesfully (with pytest)
