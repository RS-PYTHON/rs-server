...
class TestDeleteS3Files:
    ...
    async def test_delete_s3_files_empty_list(self, mocker, s3_manager):
        ...
        result = await S3Manager(s3_manager).delete_s3_files([])

        assert result is True
        mock_logger.info.assert_called_once_with("No files to be deleted from bucket")

    @pytest.mark.asyncio
    async def test_delete_s3_files_no_s3_handler(self, mocker, s3_manager):
        ...
        with pytest.raises(RuntimeError, match="Failed to create the s3 handler"):
            await S3Manager(s3_manager).delete_s3_files(["s3://bucket_name/path/to/file"])

        mock_logger.error.assert_called_once_with("Failed to create the s3 handler when trying to delete the s3 files")

    @pytest.mark.asyncio
    async def test_delete_s3_files_valid_paths(self, mocker, s3_manager):
        ...
        result = await S3Manager(s3_manager).delete_s3_files(["s3://bucket_name/path/to/file"])
    ...
    @pytest.mark.asyncio
    async def test_delete_s3_files_deletion_runtime_error(self, mocker, s3_manager):
        ...
        mock_s3_handler.adelete_keys_from_s3 = mocker.AsyncMock(side_effect=deletion_error)
        mock_get_s3_handler.return_value = mock_s3_handler
        ftbd = "s3://bucket_name/path/to/file"
        with pytest.raises(RuntimeError, match="Deletion failed"):
            await S3Manager(s3_manager).delete_s3_files([ftbd])

        mock_logger.exception.assert_called_once_with(
            "Failed to delete S3 files for catalog operation: %s",
            deletion_error,
        )
    ...
    async def test_delete_s3_files_adelete_raises_runtime_error(self, mocker, s3_manager):
        ...
        with pytest.raises(RuntimeError, match="Test failure"):
            await manager.delete_s3_files(files_to_delete)
...
class TestGetS3Handler:
    ...
    def test_update_assets_checksums_skips_asset_without_href(self, mocker, s3_manager):
        """
        Test that assets without href are skipped when adding checksum attributes.
        """
        content = {
            "collection": "test_collection",
            "properties": {"owner": "test_user"},
        ...
        mocker.patch.object(
            s3_manager.s3_handler,
            "get_object_attributes",
            return_value={"Checksum": {"ChecksumCRC32": "checksum-2"}},
        )

        result = s3_manager.update_assets_checksums(content)

        assert "file:checksum" not in result["assets"]["asset1"]
        assert result["assets"]["asset2"]["file:checksum"] == "checksum-2"
        s3_manager.s3_handler.get_object_attributes.assert_called_once_with(
            "rspython-ops-catalog-all-production",
            "file2",
        )

    def test_update_assets_checksums_skips_asset_when_checksum_lookup_fails(self, mocker, s3_manager):
        """
        Test that checksum lookup errors do not stop checksum updates for other assets.
        """
        content = {
            "collection": "test_collection",
            "properties": {"owner": "test_user"},
        ...
        mocker.patch.object(
            ...
            "get_object_attributes",
            side_effect=[
                RuntimeError("checksum unavailable"),
                {"Checksum": {"ChecksumCRC32": "checksum-2"}},
            ],
        )
