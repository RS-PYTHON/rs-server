...
def temporal_op_query(
    op: str,
    args: list[dict],
    temporal_mapping: dict[str, str],
    is_cadip_or_auxip: bool = False,
) -> str:
    ...
    outq = (
        ...
        # Note: lte and gte are currently not supported in Cadip / Auxip stations, so we use lt and gt instead
        # Whenever this gets fixed, remove the two ifs below and the "is_cadip_or_auxip" input
        .replace("<=", "lte" if not is_cadip_or_auxip else "lt")
        .replace(">=", "gte" if not is_cadip_or_auxip else "gt")
        .replace("=", "eq")
        .replace("<", "lt")
        .replace(">", "gt")
    )
