...
def prip_stac_mapper() -> dict[str, str]:
    """Return the STAC→OData field map for PRIP searches.
    Tries `prip_stac_mapper.json`; if absent, falls back to `adgs_stac_mapper.json`
    and applies PRIP-specific overrides where needed.
    """
    mapper_paths = [
        PRIP_CONFIG / "prip_stac_mapper.json",
        PRIP_CONFIG / "adgs_stac_mapper.json",  # fallback
    ]
    for mp in mapper_paths:
        if mp.exists():
            with open(mp, encoding="utf-8") as f:
                mapping: dict[str, str] = json.loads(f.read())
            break
    else:
        mapping = {}

    # ---- PRIP overrides (keep conservative; rely on ws-config to finalize) ----
    # created: PRIP ICD differs from AUXIP for "created". Prefer "published" if present.
    # We expose both keys so upstream code can build $orderby/$filter consistently.
    mapping.setdefault("published", "PublicationDate")
    # If "created" isn't explicitly mapped in a PRIP mapping file, map it to "PublicationDate" as default,
    # while the backend may choose to override in ws-config.
    mapping.setdefault("created", mapping.get("published", "PublicationDate"))
    # product:type remains an attribute-driven filter but is named "productType" in OData attributes
    mapping.setdefault("product:type", "productType")
    # Sentinel platform fields (constellation/platform)
    mapping.setdefault("constellation", "platformShortName")
    mapping.setdefault("platform", "platformSerialIdentifier")
    return mapping


def select_config(collection_id: str) -> dict[str, Any] | None:
    """Select a collection block by id from the PRIP search config."""
    return next((c for c in read_conf().get("collections", []) if c.get("id") == collection_id), None)


def stac_to_odata(stac_params: dict[str, Any]) -> dict[str, Any]:
    """Translate STAC parameter names to the PRIP OData/eodag names using the mapper."""
    m = prip_stac_mapper()
    return {m.get(k, k): v for k, v in stac_params.items()}
...
def serialize_prip_asset(feature_collection: stac_pydantic.ItemCollection, products: list[dict[str, Any]]):
    ...
    for feature in feature_collection.features:
        auxip_id = feature.properties.dict().get("auxip:id") or feature.id  # keep compatibility
        # Find matching product by id
        matched = next((p for p in products if p.get("properties", {}).get("id") == auxip_id), None)
        if not matched:
            raise HTTPException(
        ...
        if not href:
            raise HTTPException(
                ...
                detail=f"Missing download href for product {auxip_id}",
            )

        # Update asset href and rename to item id
        feature.assets["file"].href = re.sub(r"\([^\)]*\)", f"({auxip_id})", href)
        new_key = (feature.id or auxip_id).rsplit(".", 1)[0]
