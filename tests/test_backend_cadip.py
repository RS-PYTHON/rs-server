...
import asyncio
from prefect import flow, task, get_run_logger, exceptions
...
async def start_download(station, files_info, local_path, obs, idx):
    ...
    for file_id, filename in files_info.items():
        if len(obs) == 0:
            payload = {'id': file_id, 'name': filename, 'local': local_path}
        else:
            payload = {'id': file_id, 'name': filename, 'obs': obs}
        ...
        # 
        payload = {'id': file_id, 'name': filename}
        response = ""
        timeout = 10
        while response != "download_ok" and timeout > 0:
            ...
            timeout -= 0.2
        if timeout <= 0:
            logger.error("Timeout for receiving the downloaded status from server passed. \
The file %s wasn't downloaded properly", filename)
        #time.sleep(3)
        #break
        ''' 
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Start date used for time interval search"        
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Stop date used for time interval search"        
    )
