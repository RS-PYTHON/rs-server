...
-   RSPY-174: \[EODAG\] download in parallel on the same machine or
    container
...
-   RSPY-239: \[Deployment\] No JupiterHub image reachable from
    Validation platform
...
-   RSPY-170: Platform deployment and start-stop playbook failed due to
    missing credential

-   RSPY-176: Platform deployment: first application deployment
    execution failed for the step cluster-issuer

-   RSPY-177: Platform Deployment: Failed to deployed application due to
    missing parameter in group\_vars
...
-   RSPY-186: Catalog application does crash properly during
    initialization
...
-   RSPY-137: Link RS-Server frontend with Backend Catalog endpoints
    (Without UAC) - Part 3
...
-   RSPY-25: Override endpoint "publication of STAC item" to the Catalog
    backend server
...
-   RSPY-100: Link RS-Server frontend with Backend Catalog endpoints
    (Without UAC) - Part 2
...
-   RSPY-73: Link RS-Server frontend with CADIP backend endpoints
    (without UAC)

-   RSPY-78: Link RS-Server frontend with Backend Catalog endpoints
    (Without UAC) - Part 1

-   RSPY-81: Deploy keycloak on K8S cluster

-   RSPY-91: Link RS-Server frontend with ADGS backend endpoints
    (without UAC)
...
-   RSPY-134: Setup Helm Chart Releaser to use Github Pages as Helm
    chart repository
...
-   RSPY-87: Develop ADGS backend server with first endpoint "GET
    /adgs/aux/search"
