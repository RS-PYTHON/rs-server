...
async def list_cadu_handler(station: str, start_date: str = None, stop_date: str = None):
    """Docstring will be here."""
    ...
    if start_date and stop_date:
        ...
        products, number = dag_client.search(start = start_date, end = stop_date)
    return {station: prepare_products(products)}

def get_station_ws(station: str):
    """Docstring will be here."""
    stations_data = json.loads(open("CADIP/library/stations_cfg.json").read())
...
def prepare_products(products_list):
    """Docstring will be here."""
    return [product.properties['Name'] for product in products_list]